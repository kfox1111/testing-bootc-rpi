...
keepalived (VRRPv3, `advert_int 0.3` for sub-second detection, unicast between the two LAN addresses) drives `/usr/libexec/ha-router/notify.sh`, which orchestrates the following sequences:
...
| /usr/libexec/ha-router/notify.sh | keepalived state-transition orchestrator |
| /usr/libexec/ha-router/check-wan.sh | WAN carrier track_script |
...
* **Force a switchover:** `systemctl stop keepalived` on the master (start it again to preempt back — node1 has the higher priority, so it retakes master when healthy).
...
* SELinux: keepalived's notify script execs ip/dhcpcd/conntrackd/arping. If transitions fail with the services running, check `ausearch -m avc` — a small local policy module (or `semanage permissive -a keepalived_t` while debugging) may be required on a hardened base.
