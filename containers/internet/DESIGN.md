...
| /usr/bin/ha-router-render + ha-router-render.service | re-renders keepalived/conntrackd configs from the templates at every boot |
...
| Path | Purpose |
|------|---------|
| /etc/ha-router/node.env | NODE_ID, PRIORITY, HB_IP, PEER_HB_IP, LAN_IP, LAN_VIP, WAN_MAC — **the only durable per-node state** |
| /etc/keepalived/keepalived.conf | rendered from template — regenerated at every boot by ha-router-render.service |
| /etc/conntrackd/conntrackd.conf | rendered from template — regenerated at every boot by ha-router-render.service |
...
Rendered configs are regenerated from the image's templates on every boot (before keepalived/conntrackd start). This matters under bootc: /etc is machine-local, so without the re-render an image update that changes a template or moves a script path would leave the node running a stale config — this bit the first hardware deployment (keepalived.conf pointed at a script location the new image no longer had). Do not hand-edit the rendered files; change node.env (then reboot or run `ha-router-render && systemctl restart keepalived conntrackd`) or the templates in the image.
